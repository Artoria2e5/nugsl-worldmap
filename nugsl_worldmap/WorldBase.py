...
class worldBase:
    def __init__(self):
        ...
        self.r_ptrans = re.compile(
            '(<path[^a-z0-9])([^>]*)transform="translate\(' + r_coords + '\)"\s+(.*)(/>)', re.M | re.S
        )
    ...
    def flatness(self, data):
        self.isflat = True
        fixer = tagFix()
        fixer.tagfix("path", data, regex='(?m)(?s).*id="ocean".*', matchfunc=self.flatness_check1)
        return self.isflat
    ...
    def apply_translations(self, data):
        ...
        """
        fixer = tagFix()
        data = fixer.tagfix("path", data, matchfunc=self.perform_translation, regex=self.r_ptrans)
        return fixer.tagfix("g", data, matchfunc=self.perform_translation, regex=self.r_group_head)
